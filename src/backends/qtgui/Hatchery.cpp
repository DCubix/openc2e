...
Hatchery::Hatchery(QtOpenc2e *parent) : QDialog(parent) {
	...
	if (engine.version == 1)
	  {
	    hatcherybgfile = world.findFile("hatchery/hatchery.bmp");

	  }
	else if(engine.version == 2)
	  {
	     hatcherybgfile = world.findFile("Applet Data/Hatchery.bmp");
	  }
	...
	if (engine.version == 1) {
	  ...
	  /* mask which goes over the eggs */
		...
		/* fan animation */
		std::string fananim[4] = {"hatchery/FAN0.BMP", "hatchery/FAN1.BMP", "hatchery/FAN2.BMP", "hatchery/FAN3.BMP"};
        
		for (unsigned int i = 0; i < 4; i++)
		{
		  QPixmap fananimation(QString(fananim[i].c_str()));
		  QGraphicsPixmapItem *fan = graphicsScene->addPixmap(fananimation);
		  fan->setPos(220, 20);
		}
